...
for a = 1:numel(andersonNames)
    ...
    plotComparisonPanel(1, aSusc, D_susc_arnold, ...
        ...
        labels, trivialAcrossArnold, opts.ZThreshold, ...
        sprintf('D(\\rightarrow i): %s vs Arnold mean \\pm SD (n=%d)', ...
            strrep(aname, '_', '\_'), nArnold), ...
        'D(\rightarrow i)');

    plotComparisonPanel(2, aInfl, D_infl_arnold, ...
        D_infl_arnold_mean, D_infl_arnold_std, zInfl, ...
        labels, trivialAcrossArnold, opts.ZThreshold, ...
        sprintf('D(k \\rightarrow): %s vs Arnold mean \\pm SD (n=%d)', ...
            strrep(aname, '_', '\_'), nArnold), ...
        'D(k \rightarrow)');
    ...
    if opts.SaveResults
        ...
        outMask = ~trivialAcrossArnold & ...
            ((~isnan(zSusc) & abs(zSusc) >= opts.ZThreshold) | ...
             (~isnan(zInfl) & abs(zInfl) >= opts.ZThreshold));
        outIdx = find(outMask);
        if ~isempty(outIdx)
            T = table(outIdx, string(labels(outIdx)), ...
                ...
                'VariableNames', {'NodeIdx','Region', ...
                    'D_susc_anderson','D_susc_arnold_mean','D_susc_arnold_std','z_susc', ...
                    'D_infl_anderson','D_infl_arnold_mean','D_infl_arnold_std','z_infl'});
            % Sort by largest absolute deviation in either centrality
            maxAbsZ = max(abs(T.z_susc), abs(T.z_infl), 'omitnan');
            [~, ord] = sort(maxAbsZ, 'descend');
            T = T(ord, :);
            writetable(T, fullfile(resultsDir, [baseName '_outliers.csv']));
            fprintf('  %s: %d outlier node(s) (|z| >= %.2f) -> %s\n', ...
                aname, numel(outIdx), opts.ZThreshold, [baseName '_outliers.csv']);
        else
            fprintf('  %s: no outlier nodes at |z| >= %.2f.\n', aname, opts.ZThreshold);
        end
    end
end
...
function plotComparisonPanel(panelIdx, anderVals, arnoldVals, ...
    arnMean, arnStd, zScore, labels, trivial, zThresh, panelTitle, ylab)
...
% Highlight outliers (|z| >= threshold and not trivial)
outlierIdx = find(~trivial & ~isnan(zScore) & abs(zScore) >= zThresh);
